	...
	<li>
		<a
			...
			(page.route.id?.startsWith(route) && route !== '/')
				...
				: ''} border-surface-400 dark:border-surface-200 transition-border bg-surface-200 dark:bg-surface-800 card card-hover inline-flex h-full w-full items-center self-stretch px-7 py-2 font-sans text-lg font-semibold duration-75 hover:border-b-3"
		>
			{label}
		</a>
	</li>
...
<AppBar class="bg-surface-200 dark:bg-surface-800 fixed top-0 z-50 h-[var(--header-height)]">
	<AppBar.Toolbar class="justify-apart h-full grid-cols-[auto_1fr_auto]">
		...
		<AppBar.Trail class="flex justify-end">
			<ul class="inline-flex items-center gap-2 pr-5">
				<li class="ml-5">
